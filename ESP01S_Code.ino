...
//const char* ssid1     = "duco-milrotik";        //<-- ใส่ชือ Wifi หลัก ตรงนี
//const char* password1 = "1234567890";    //<-- ใส่รหัส Wifi หลัก ตรงนี

//const char* ssid2     = "duco-milrotik";        //<-- ใส่ชือ Wifi รอง ตรงนี ถ้าใช้ WiFi ตัวเดียวก็ให้ใส่เหมือนกันกับ ssid1
//const char* password2 = "1234567890";    //<-- ใส่รหัส Wifi รอง ตรงนี

const char* ssid2     = "MY_IOT";        //<-- ใส่ชือ Wifi หลัก ตรงนี
const char* password2 = "0819065291";    //<-- ใส่รหัส Wifi หลัก ตรงนี

const char* ssid1     = "duco-milrotik";        //<-- ใส่ชือ Wifi รอง ตรงนี ถ้าใช้ WiFi ตัวเดียวก็ให้ใส่เหมือนกันกับ ssid1
const char* password1 = "1234567890";    //<-- ใส่รหัส Wifi รอง ตรงนี
...
String BrdNm          = "ESP01-B0-66"; //<-- ใส่ชือ RigName ตรงนี
...
const String FirmwareVer={"1.1"}; // Last of Version
#define URL_fw_Version "/Thanormsin/FwESP01S/master/bin_version.txt"
#define URL_fw_Bin "https://raw.githubusercontent.com/Thanormsin/FwESP01S/master/fw01.bin"
...
const char * host2 = "51.15.127.80"; // Static server IP
const int port7 = 2813;
const int port6 = 2811;
const int port5 = 2812;
const int port4 = 2810;
const int port3 = 2820;
...
int i;
  for (i = 1; i <= 30; i++) {
    ...
    if (!client.connected()) {
      client.connect(host1, port2);
      port = port2;
      delay(8000);
    }
    if (!client.connected()) {
      client.connect(host2, port3);
      port = port3;
      delay(8000);
    }
    if (!client.connected()) {
      client.connect(host2, port4);
      port = port4;
      delay(8000);
    }
    if (!client.connected()) {
      client.connect(host2, port5);
      port = port5;
      delay(8000);
    }
    if (!client.connected()) {
      client.connect(host2, port6);
      port = port6;
      delay(8000);
    }
    if (!client.connected()) {
      client.connect(host2, port7);
      port = port7;
      delay(8000);
    }
  }
